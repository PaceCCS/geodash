...
import { c as cn } from "./router-kgXxnTvr.js";
...
async function writeNetworkFile(path, content) {
  return invoke("write_network_file", { path, content });
}
...
async function exportNetworkToToml(nodes, edges, directoryPath) {
  ...
  await Promise.all(
    nodes.map(async (node) => {
      ...
      const outgoing = networkNode.type === "branch" ? (edgesBySource.get(node.id) ?? []).map((e) => ({
        target: e.target,
        weight: e.data.weight
      })) : void 0;
      const tomlContent = serializeNodeToToml(networkNode, outgoing);
      await writeNetworkFile(`${base}${node.id}.toml`, tomlContent);
    })
  );
}
...
async function getNetworkFromPath(networkIdentifier) {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      body.error ?? `Failed to load network (${response.status})`
    );
  }
...
function useFileWatcher() {
  ...
  useEffect(() => {
    ...
    const unlistenPromise = listen("file-changed", async (event) => {
