...
export function PointRecordTable({
  ...
  onCellChange: (rowIndex: number, fieldIndex: number, value: ShapefileCell) => void;
}) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-3 rounded-lg border border-border/80 bg-muted/20 px-4 py-3 md:flex-row md:items-center md:justify-between">
        ...
        >
          Showing {totalRows === 0 ? 0 : startIndex + 1}-{endIndex} of {totalRows} points
        </p>
      ...
      <table className="min-w-full border-separate border-spacing-0">
        ...
        <tbody>
          {visibleRecords.map((record, visibleIndex) => {
            ...
            return (
              <tr key={`${record.number}-${rowIndex}`}>
                <td className="border-b border-border px-3 py-3 text-sm text-muted-foreground">
                  {record.number}
                </td>
                {(["x", "y", "z", "m"] as const).map((key) => (
                  <td key={key} className="border-b border-border px-3 py-3 align-top">
                    <NumericInput
                      value={geometry[key]}
                      onChange={(value) => onPointChange(rowIndex, key, value)}
                    />
                  </td>
                ))}
                {document.fields.map((field, fieldIndex) => (
                  ...
                  >
                    <DbfCellEditor
                      ...
                      onChange={(value) => onCellChange(rowIndex, fieldIndex, value)}
                    />
                  </td>
                ))}
                <td className="border-b border-border px-3 py-3 text-right align-top">
                  <Button variant="ghost" size="sm" onClick={() => onDeleteRow(rowIndex)}>
