...
const FlowPath = struct {
    ...
    fluid: []const u8 = "",
    pipes: std.ArrayListUnmanaged(PipeSegment) = .empty,
    sources: std.ArrayListUnmanaged(SourceDef) = .empty,
    compressors: std.ArrayListUnmanaged(CompressorDef) = .empty,
...
fn splitStatements(allocator: Allocator, text: []const u8) ![][]const u8 {
    var stmts = std.ArrayListUnmanaged([]const u8).empty;
...
pub fn parseKey(
    ...
    var flow_paths = std.ArrayListUnmanaged(FlowPath).empty;
    var nodes = std.ArrayListUnmanaged(NodeDef).empty;
    var connections = std.ArrayListUnmanaged(Connection).empty;
    var fluids = std.ArrayListUnmanaged(FluidDef).empty;
    ...
    var outgoings = std.ArrayListUnmanaged(Outgoing).empty;
    ...
    for (flow_paths.items, 0..) |fp, fi| {
        ...
        var block_arr = Value.Array.empty;
...
pub fn writeKey(
    ...
    try out.print(allocator, "! Generated by geodash\nCASE PROJECT='geodash', TITLE='{s}'\n\n", .{network.label});

    try out.appendSlice(allocator, "OPTIONS COMPOSITIONAL=OFF, TEMPERATURE=WALL\n");
    try out.appendSlice(allocator, "TIME ENDTIME=3600, DTSTART=1, DTMAX=60\n");
    try out.appendSlice(allocator, "INTEGRATION MAXITERATIONS=20, MAXLOCALERROR=1e-3\n\n");

    try out.appendSlice(allocator, "MATERIAL LABEL=WALL-1, CONDUCTIVITY=50, CAPACITY=500, DENSITY=7850\n");
    try out.appendSlice(allocator, "WALL LABEL=WALL-1, THICKNESS=(0.02), MATERIAL=(WALL-1)\n\n");

    // Collect all fluid labels from Source blocks
    var fluid_labels = std.ArrayListUnmanaged([]const u8).empty;
    ...
    if (fluid_labels.items.len == 0) {
        ...
        try out.appendSlice(allocator, "SINGLEOPTIONS COMPONENT=CO2\n\n");
    } else {
        for (fluid_labels.items) |fl| {
            try out.print(allocator, "FEED LABEL={s}, COMPOSITION=(CO2 1.0)\n", .{fl});
        }
        try out.append(allocator, '\n');
    }
    ...
    for (network.nodes.items, 0..) |*node, ni| {
        switch (node.*) {
            .branch => |*br| {
                ...
                try out.print(allocator, "NETWORKCOMPONENT TYPE=FlowPath, TAG=FP_{s}\n", .{br_id});

                const br_label = if (br.base.label) |l| l else br_id;
                try out.print(allocator, "  BRANCH LABEL='{s}', GEOMETRY=GEOM-{d}, FLUID={s}\n", .{ br_label, ni + 1, fluid_label });
                try out.print(allocator, "  GEOMETRY LABEL=GEOM-{d}\n", .{ni + 1});
                ...
                if (br.base.extra.get("ambient_temperature")) |atv| {
                    if (atv.getFloat()) |at| {
                        try out.print(allocator, "  HEATTRANSFER PIPE=ALL, TAMBIENT={d:.2}\n", .{at});
                    }
                }

                try out.appendSlice(allocator, "ENDNETWORKCOMPONENT\n\n");
            },
            else => {},
        }
    }
        ...
        switch (node.*) {
            .branch => |*br| {
                ...
                if (!has_incoming.contains(ni)) {
                    try out.print(allocator, "NETWORKCOMPONENT TYPE=Node, TAG=NODE_INLET_{s}\n", .{br_id});
                    try out.print(allocator, "  PARAMETERS LABEL=INLET_{s}, TYPE=CLOSED\n", .{br_id});
                    try out.appendSlice(allocator, "ENDNETWORKCOMPONENT\n\n");
                }
                ...
                for (br.blocks.items, 0..) |*block, bi| {
                    ...
                    const temperature = if (block.extra.get("temperature")) |v| v.getFloat() orelse 20 else 20;
                    try out.print(allocator, "NETWORKCOMPONENT TYPE=Node, TAG=NODE_OUTLET_{s}_{d}\n", .{ br_id, bi });
                    try out.print(
                        allocator,
    ...
    for (network.edges.items) |*edge| {
        try out.print(allocator, "NETWORKCOMPONENT TYPE=Node, TAG=NODE_{s}_{s}\n", .{ edge.source, edge.target });
        try out.print(allocator, "  PARAMETERS LABEL=JUNC_{s}_{s}\n", .{ edge.source, edge.target });
        try out.appendSlice(allocator, "ENDNETWORKCOMPONENT\n\n");
    }
...
pub fn computeRouteSegmentsFromShp(
    ...
    var points = std.ArrayListUnmanaged(shapefile.PointZ).empty;
